...
import { Loader2, Printer, ArrowLeft, ClipboardList, AlertCircle } from 'lucide-react';
...
export default function AlbaranesPage() {
  ...
  const [albaranes, setAlbaranes] = useState<GroupedDocument[]>([]);
  const [selectedAlbaran, setSelectedAlbaran] = useState<GroupedDocument | null>(null);
    ...
    if (user?.usuari) {
      const fetchData = async () => {
        ...
        try {
          ...
          const processedAlbaranes: GroupedDocument[] = Object.values(grouped).map((doc: any) => {
              const subtotal = doc.lines.reduce((sum: number, line: any) => sum + line.netTotal, 0);
              const vatDetails = doc.lines.reduce((acc: Record<string, VatDetail>, line: any) => {
              ...
              return { ...doc, subtotal, vatDetails, total };
          });

          setAlbaranes(processedAlbaranes.sort((a, b) => {
            try {
          ...
          setError(e.message || 'Ha habido un problema al cargar tus albaranes. Por favor, inténtalo de nuevo más tarde.');
          console.error(e);
        } finally {
          setIsLoading(false);
        }
  ...
  if (selectedAlbaran) {
    const { clientData } = selectedAlbaran;
    return (
      <div className="bg-background print:bg-white">
        <div className="container mx-auto px-4 py-8 print:p-0">
          <div className="mb-8 flex justify-between items-center print:hidden">
            <Button variant="outline" onClick={() => setSelectedAlbaran(null)}>
          ...
          <div id="zona-factura" className="bg-card text-card-foreground p-8 sm:p-12 rounded-lg border shadow-lg print:border-none print:shadow-none print:rounded-none">
            <header className="flex justify-between items-start pb-8 border-b">
              <div>
                <h1 className="text-3xl font-bold text-foreground">ALBARÁN</h1>
                <p className="text-muted-foreground">Nº Albarán: {selectedAlbaran.id}</p>
                <p className="text-muted-foreground">Fecha: {formatDate(selectedAlbaran.date)}</p>
                <Badge className={cn(
                    "mt-2",
                    selectedAlbaran.status === 'Pagada' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                )}>
                    {selectedAlbaran.status}
                </Badge>
              </div>
              <Logo />
            </header>
            ...
            <section className="grid sm:grid-cols-2 gap-8 my-8">
              ...
              <div>
                ...
                {clientData ? (
                  ...
                  <p className="text-sm text-destructive">Datos del cliente no disponibles.</p>
                )}
              </div>
            </section>
            ...
            <section>
              <Table>
                ...
                <TableBody>
                  {selectedAlbaran.lines.map((line, index) => (
                    <TableRow key={index}>
            ...
            <section className="flex flex-col items-end mt-8">
              <div className="w-full max-w-sm space-y-4">
                  <div className="space-y-2 border-t pt-4">
                      {Object.entries(selectedAlbaran.vatDetails).map(([rate, details]) => (
                ...
                <div className="flex justify-between text-lg font-bold border-t pt-2 mt-2">
                  ...
                  <span className="text-primary">{selectedAlbaran.total.toFixed(2)} €</span>
                </div>
              </div>
            </section>
            
            <footer className="mt-12 pt-4 border-t text-xs text-muted-foreground space-y-4">
                {selectedAlbaran.paymentMethod && <p><strong>Forma de pago:</strong> {selectedAlbaran.paymentMethod}</p>}
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {albaranes.map(albaran => (
                        <Card key={albaran.id} className="flex flex-col">
                            <CardHeader>
                                <div className="flex flex-row items-start justify-between">
                                    <div>
                                        <CardTitle className="text-lg">Albarán {albaran.id}</CardTitle>
                                        <CardDescription>{formatDate(albaran.date)}</CardDescription>
                                    </div>
                                ...
                                <div className="pt-2">
                                     <Badge className={cn(
                                        "font-semibold",
                                        albaran.status === 'Pagada' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                    )}>
                                        {albaran.status}
                                    </Badge>
                                </div>
                            ...
                            <CardContent className="flex-grow flex flex-col justify-end">
                                <p className="text-3xl font-bold text-right text-primary mb-4">{albaran.total.toFixed(2)} €</p>
                                <Button className="w-full" onClick={() => setSelectedAlbaran(albaran)}>
                                    Ver Albarán
                                </Button>
                            </CardContent>
                        </Card>
                ...
                <div className="text-center py-10 max-w-2xl mx-auto">
                     <Card className="p-8">
                        <CardHeader>
                            <CardTitle>No hay albaranes</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-muted-foreground">De momento no tienes ningún albarán generado. Cuando tengas uno, aparecerá aquí.</p>
                        </CardContent>
                    </Card>
                </div>
            )}
